<template>
    ...
    <div v-else class="container-fluid pt-5 h-100 d-flex flex-column align-items-center justify-content-center">
        <h1 v-if="newChallenge.Name == '' || newChallenge.Name == undefined"> Docker Challenge </h1>
        <h1 v-else> {{newChallenge.Name}} </h1>
        <section class="w-50" v-if="currentStep==1">

            <div class="mb-3">
                <label for="cn" class="form-label">Name</label>
                <input type="text" class="form-control disabled" id="cn" placeholder="Challenge Name" v-model="newChallenge.Name" required :disabled="isEditChallenge">
            </div>
            ...
            <div class="mb-3">
                ...
                <select v-model="newChallenge.RequiredChallengeID" class="form-select" id="crc" multiple>
                    <option v-for="challenge in Challenges" :value="challenge.Id" :key="challenge.Id">
                        {{ challenge.Name }}
                    </option>
                </select>
        ...
        <section class="w-50" v-else-if="currentStep == 2">
            ...
            <div class="mb-3" v-if='!isEditChallenge'>
                <label for="ca" class="form-label">Add your Attachments</label>
                <input class="form-control" accept=".tar" type="file" id="ca" @change="onFileUpload($event)">
            </div>
        ...
        <section v-else-if="currentStep == 3" class="w-50 h-100">
        ...
        <section class="w-50" v-else-if="!isEditChallenge">

            <div class="d-flex flex-row my-5">
                <div class="input-group">
                    <div class="input-group-prepend">
                        <select class="form-control" v-model="newPortMapping.Protocol">
                            <option value="tcp">TCP</option>
                            <option value="udp">UDP</option>
                            <option value="sctp">SCTP</option>
                        </select>
                    </div>
                    <input type="number" class="form-control" placeholder="8080" v-model="newPortMapping.Port">
                    <button class="btn btn-outline-light" type="button" @click="onAddPortMapping">Add</button>
                </div>
            </div>
            ...
            <div class="container-fluid">
                <div 
                    class="card my-3 text-dark fw-bolder"
                    v-for="(portMapping, index) in PortMappings"
                    :key="index"
                >
                    <div class="card-body d-flex flex-row justify-content-between">
                        <span>
                        ...
                        <div class="actions">
                            <BIconTrash class="mx-1 pointer" v-on:click="onDeletePortMapping(index)"></BIconTrash>
                        </div>
                    </div>
                </div>
            </div>
...
import { BIconTrash, BIconArrowDown, BIconArrowUp } from 'bootstrap-icons-vue'
...
export default defineComponent ({
    components : { ButtonComponent,  BIconTrash, BIconArrowDown, BIconArrowUp },
    setup () {
        const toast = useToast();
        const store = useStore<State>();
        const route = useRoute();
        const isLoading = ref<boolean>(false);
        const maxSteps = ref<number>(4);
        const currentStep = ref<number>(1);

        const newPortMapping = reactive<PortMapping>({Port: 0, Protocol : 'tcp'});

        const PortMappings = reactive<PortMapping[]>([]);
        ...
        const onNextStep = () => {
            ...
            currentStep.value = (currentStep.value + 1) % (maxSteps.value + 1);
        }; 
        ...
        const onAddPortMapping = () => {
            const nPm : PortMapping = {Port : newPortMapping.Port, Protocol : newPortMapping.Protocol}
            PortMappings.push(nPm);
            newPortMapping.Port = 0;
        ...
        const onCreateChallenge = async () => {
            try {
                ...
                PortMappings.forEach(p => ps.push(`${p.Port}/${p.Protocol}`) )

                const nc : Challenge = {ChallengeType : "Dockerized", Challenge: newChallenge.value};
                nc.Challenge.Ports = ps;
        ...
        const onDeletePortMapping = (index : number) => PortMappings.splice(index, 1);
